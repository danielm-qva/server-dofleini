...
export class ControllerPost {
  ...
  public createPost = async (req: Request, res: Response) => {
    const { description, photoPost, user , title } = req.body;
    const newPost = new Post(title, description, photoPost, user);
  ...
  public getAllMyPosy = async (req: Request, res: Response) => {
    const { id } = req.params;
    console.log();
    try {
      this.repo.FindMyPost(parseInt(id)).then(ans => {
